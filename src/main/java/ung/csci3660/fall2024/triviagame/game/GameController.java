...
public class GameController {
    ...
    public boolean initialize(GameConfig config) {
        if (session.getState() != GameState.INITIALIZING) return false;
    ...
    public boolean startGame() {
        if (session.getState() != GameState.READY) return false;
    ...
    public GameResult submitAnswer(String answer) {
        if (session.getState() != GameState.IN_PROGRESS) return null;
    ...
    public boolean moveToNextQuestion() {
        if (session.getState() != GameState.IN_PROGRESS) return false;
